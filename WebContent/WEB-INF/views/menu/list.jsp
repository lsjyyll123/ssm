...
<div class="easyui-layout" data-options="fit:true">
    ...
    <div id="wu-toolbar">
        ...
        <div class="wu-toolbar-search">
            ...
            <label> <a href="#" id="export-btn"  onclick="exportExcle()"  class="easyui-linkbutton" iconCls="icon-arrow-switch">导出Excel</a></label>
            <label> <a href="#" id="print-btn" onclick="printPdf()" class="easyui-linkbutton" iconCls="icon-image-link">打印</a></label>
        </div>
    </div>
    <!-- End of toolbar -->
...
<div id="edit-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:450px; padding:10px;">
	<form id="edit-form" method="post">
        ...
        <table>
            ...
            <tr>
                <td align="right" id="isevntMenu">菜单URL:</td>
	...
	<form id="add-menu-form" method="post">
        <table>
            ...
            <tr>
                <td align="right">上级菜单:</td>
                <td>
                	<input type="hidden" name="_parentId" id="--" value="0">
...
<!-- 选择图标弹窗 -->
<div id="select-icon-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:920px;height:500px; padding:10px;">
...
function exportExcle(){
	//$('#data-datagrid').datagrid('toExcel', 'data-datagrid.xls');
}
function printPdf(){
//	$('#data-datagrid').datagrid('print', 'DataGrid');
}
	/**
	*  添加记录
	*/
				...
				success:function(data){
					if(data.type == 'success'){
						...
						for(var i=0;i<icons.length;i++){
							var tbody = '<td class="icon-td"><a onclick="selected(this)" icon="' + icons[i] + '" href="javascript:void(0)" style="">'+
								'<div class="' + icons[i] + '" style="text-align:center;height:22px;width:95px;display:block;"></div>'
								+'<div style="height:45px;width:95px;display:block;word-break: normal;text-align:center;">'+icons[i]+'</div></a></td>';
							if(i == 0){
								table += '<tr>' + tbody;
								continue;
							}
	...
	function openEdit(){
		...
		$('#edit-dialog').dialog({
            ...
            onBeforeOpen:function(){
            	...
            	$("#isevntMenu").html("菜单URL:");
            	//判断是否是按钮
            	var parent = $('#data-datagrid').treegrid('getParent',item.id);
        		if(parent != null){
        			if(parent.parentId != 0){
            			$("#isevntMenu").html("菜单事件:");
